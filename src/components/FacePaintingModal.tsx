...
export function FacePaintingModal({ isOpen, onClose }: FacePaintingModalProps) {
  ...
  const handleBookWhatsApp = () => {
    const phoneNumber = "257XXXXXXXX"; // Votre numéro WhatsApp sans le '+'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm animate-fadeIn">
      <div 
        className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-[#1C1B18]/20 p-6 shadow-2xl md:p-8"
        style={{ backgroundColor: "#C3C2A9" }}
      >
        {/* Bouton Fermer */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 text-2xl font-bold text-[#1C1B18] transition-opacity hover:opacity-70"
        >
          ✕
        </button>

        {/* Carrousel d'images (Slider) */}
        <div className="relative h-64 md:h-80 w-full overflow-hidden rounded-lg border border-[#1C1B18]/15 bg-black/10">
            ...
            {GALLERY_IMAGES.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  idx === currentSlide ? "w-6 bg-[#D4AF37]" : "w-2 bg-white/60"
                }`}
              />
            ))}
          </div>
        ...
        <div className="mt-6 space-y-4 text-[#1C1B18]">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#1C1B18]/15 pb-3">
            <h2 className="font-serif text-2xl md:text-3xl font-bold">
              Event Face Painting
            </h2>
            <span className="rounded bg-[#1C1B18] px-3 py-1 text-xs font-bold text-[#F0EFDF] uppercase tracking-wider">
              Service Sur-Mesure
            </span>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 pt-2">
            <div className="rounded border border-[#1C1B18]/15 bg-white/30 p-3">
              <h4 className="text-xs font-bold uppercase tracking-wider mb-1">🌿 Produit Sûr & Doux</h4>
              <p className="text-[11px] text-[#2C2B26]">
                Utilisation exclusive de produits de qualité cosmétique, hypoallergéniques et nettoyables simplement à l'eau.
              </p>
            </div>

            <div className="rounded border border-[#1C1B18]/15 bg-white/30 p-3">
              <h4 className="text-xs font-bold uppercase tracking-wider mb-1">✨ Pour Tous Vos Événements</h4>
              <p className="text-[11px] text-[#2C2B26]">
